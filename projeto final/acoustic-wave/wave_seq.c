...
void save_grid(int rows, int cols, int time, float *matrix){
    ...
    char file_name[300];
    char img_name[64];
    sprintf(img_name, "wavefield_seq_%d_%d_%d", rows, cols, time);
    sprintf(file_name, "wavefield/wavefield_seq_%d_%d_%d.txt", rows, cols, time);
    ...
    char command[400];
    sprintf(command, "python3 plot.py --file '%s' --name %s", file_name, img_name);
    printf("%s\n", command);
